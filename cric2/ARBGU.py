...
start_15m = str(pd.Timestamp.today() + pd.DateOffset(-10))[0:10]
...
def Finder_IETE(time1,time_name1,start1,time2,time_name2,start2):
        while True:
                ...
                with my_lock:
                        ...
                        api_url_livePrice = f'https://api.polygon.io/v2/last/trade/{tiker_live}?apiKey={api_key}'
                        data = requests.get(api_url_livePrice, verify=False).json()
...
#
                ...
                print('--- Mode recherche ARBGU',time1,time_name1,' ---', flush=True)
                ...
                if (C - B) < (C - D) and (C - B) < (E - D) and (E - F) < (E - D) and (E - F) < (C - D) and B > D and F > D and B < C and F < E and A >= mirande2['c'].iloc[0] and verif == 0 and local_min[-1] > local_max[-1]:
                        try:
                                J = line_intersection((AJ, BJ), (CJ, DJ))
                                I = line_intersection((AI, BI), (CI, DI))
                                accept = True
                        ...
                        if I[1] > B and J[1] > F and moyenne_epaule1 <= moyenne_tete / 2 and moyenne_epaule2 <= moyenne_tete / 2 and moyenne_epaule1 >= moyenne_tete / 4 and moyenne_epaule2 >= moyenne_tete / 4 and df['c'].values[-1] <= J[1] + (moyenne_tete) / 4 and df['c'].values[-1] >= J[1]  and df['c'].values[-2] <= J[1] + (moyenne_tete) / 4 and df['c'].values[-2] >= J[1]  and G >= J[1] and accept == True:
